...
public class TargetListMapper {
    ...
    public StudentTargetResponseDto toStudentTargetResponseDto(StudentTargetDataResponseDto studentData) {
        return StudentTargetResponseDto.builder().id(studentData.id()).student(studentData).build();
    }
    ...
    public List<StudentTargetResponseDto> mapHofEntriesToStudentTargets(Page<HallOfFameEntry> hofEntries) {
        return hofEntries.stream().map(this::mapHofEntryToStudentTarget).toList();
    }
