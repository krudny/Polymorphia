...
public interface GradableEventRepository extends JpaRepository<GradableEvent, Long> {
    ...
    @Query("""
                    select hofe.studentId as studentId, hofe.studentName as fullName, hofe.animalName as animalName,
                            hofe.evolutionStage as evolutionStage, hofe.groupName as group, hofe.imageUrl as imageUrl,
                     ...
                     where (scga.courseGroup.teachingRoleUser.userId = :teachingRoleUserId
                        or scga.courseGroup.course.coordinator.userId = :teachingRoleUserId)
            
                       and (:includeAllGroups = true or hofe.groupName in :groups)
            
                       and (
                             :searchTerm = '' or (
                                  (:searchByAnimal = true and lower(hofe.animalName) like lower(concat('%', :searchTerm, '%')))
                               or (:searchByStudent = true and lower(hofe.studentName) like lower(concat('%', :searchTerm, '%')))
                             )
                       )
                    and (
                           :gradeStatus = 'ALL'
                           or (:gradeStatus = 'GRADED' and g.id is not null)
                           or (:gradeStatus = 'UNGRADED' and g.id is null)
                    )
            
                     group by hofe.studentId, hofe.studentName, hofe.animalName, hofe.evolutionStage,
                              hofe.groupName, hofe.imageUrl
                      order by
                         case when :sortBy = 'gainedXp' and :sortOrder = 'ASC' then COALESCE(sum(cg.xp), -1) end asc,
                         case when :sortBy = 'gainedXp' and :sortOrder = 'DESC' then COALESCE(sum(cg.xp), -1) end desc,
