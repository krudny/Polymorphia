...
ALTER TABLE students_course_groups
ADD CONSTRAINT fk_students_course_groups_on_animal FOREIGN KEY (animal_id) REFERENCES animals (id) ON DELETE SET NULL;

ALTER TABLE students_course_groups
ADD CONSTRAINT fk_students_course_groups_on_course_group FOREIGN KEY (course_group_id) REFERENCES course_groups (id) ON DELETE CASCADE;

ALTER TABLE students_course_groups
ADD CONSTRAINT fk_students_course_groups_on_student FOREIGN KEY (student_id) REFERENCES students (user_id) ON DELETE CASCADE;
...
WITH
  ...
  awarded AS (
    ...
    WHERE
      it.event_section_id = ge.event_section_id
  ),
  bonuses AS (
    SELECT
      a.animal_id,
...
SELECT
  (
    b.animal_id::text || '-'::text || b.event_section_id::text
  ) AS id,
  b.student_id,
  b.animal_id,
  b.event_section_id,
...
FROM
  base b
  LEFT JOIN points p ON p.animal_id = b.animal_id
  AND p.event_section_id = b.event_section_id
  LEFT JOIN bonuses bn ON bn.animal_id = b.animal_id
  AND bn.event_section_id = b.event_section_id;
...
WITH
  ...
  evolution_mapping AS (
    ...
    FROM
      ranked r
      LEFT JOIN evolution_stages es ON es.course_id = r.course_id
