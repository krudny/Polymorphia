...
public class HallOfFameSortSpecResolverTest {
    ...
    void resolve_shouldReturnOverviewFieldSort(
        String sortBy,
        String expectedDbField
    ) {
        // Given
        HallOfFameRequestDto requestDto = testHallOfFameRequestDto(sortBy);

        // When
        HallOfFameSortSpec result = hallOfFameSortSpecResolver.resolve(
            requestDto
        );

        // Then
        assertThat(result).isInstanceOf(OverviewFieldSort.class);
        assertThat(((OverviewFieldSort) result).field()).isEqualTo(
            expectedDbField
        );
    ...
    void resolve_shouldReturnEventSectionSort() {
        // Given
        HallOfFameRequestDto requestDto = testHallOfFameRequestDto(
            EVENT_SECTION_NAME
        );

        // When
        when(
            eventSectionRepository.existsByCourseIdAndName(
                COURSE_ID,
                EVENT_SECTION_NAME
            )
        ).thenReturn(true);
    ...
    void resolve_shouldThrow_forInvalidSortBy() {
        ...
        assertThrows(IllegalArgumentException.class, () ->
            hallOfFameSortSpecResolver.resolve(requestDto)
        );
    }
